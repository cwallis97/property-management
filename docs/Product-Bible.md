...
- **Work Order Assignment / My Work.** Property Access V1 is deliberately designed for this to layer on cleanly next (see that entry above for the exact planned semantics), but no `assignedToUserId` column, assignment UI, or Technician mutation capability exists yet.
