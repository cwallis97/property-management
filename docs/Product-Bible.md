...
- **Appearance preference (state/UI only — no styling yet)** — a System/Light/Dark selector lives in Account, backed by a new `ThemeContext` and persisted to `localStorage` only (no `User` column, no migration — deliberately device-local, since nothing yet gives a theme preference a reason to follow someone between devices). "System" tracks `prefers-color-scheme` live via a `matchMedia` listener. A small inline script in `index.html` stamps `data-theme` on `<html>` synchronously before React paints, avoiding a flash of the wrong theme. **Important: no CSS or Tailwind class in the app reacts to `data-theme` yet** — this milestone is the preference architecture only. The actual dark-mode visual conversion (semantic color tokens, per-component styling) is an explicit, separate next milestone, not yet started.
...
- **Dark Mode styling.** The Appearance preference above only sets state/attributes — no page, component, badge, or table has been visually converted to respond to it. Every screen still renders in its single, light-only palette regardless of the selected preference.
