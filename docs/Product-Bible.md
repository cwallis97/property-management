...
- **Ownership transfer, Property-level user access, Regional Manager role, Property Manager-specific access, a Leasing Agent role, a Maintenance Supervisor role, custom/configurable roles, or a configurable permission matrix.** None of these exist. The ROLE → CAPABILITY architecture is deliberately structured so each can be added later without redesigning it.
...
- **Settings → Users & Roles management UI, invitations, Property-level user access, a Regional Manager role, Property Manager-specific access, a Leasing Agent role, a Maintenance Supervisor role, custom/configurable roles, or a configurable permission matrix.** Roles & Permissions V1 is Company-level only — every Company member currently sees every Property the Company owns; nothing yet restricts which Properties a specific user may access, and there is no UI anywhere to view or change another user's role.
